...
# generate data and vocab
    ...
    vocab.sort()
    with open('./20news-bydate/vocab-raw.txt', 'w') as f:
        f.write('\n'.join(vocab))
    test_data = collect_data_from(parent_path= test_path, newsgroup_list= newsgroup_list) 
    with open('./20news-bydate/20news-train-raw.txt', 'w') as f:
        f.write('\n'.join(train_data))
    with open('./20news-bydate/20news-test-raw.txt', 'w') as f:
        f.write('\n'.join(test_data))
...
# gen_data_and_vocab()
encode_data('./20news-bydate/20news-train-raw.txt', './20news-bydate/vocab-raw.txt')
encode_data('./20news-bydate/20news-test-raw.txt', './20news-bydate/vocab-raw.txt')
